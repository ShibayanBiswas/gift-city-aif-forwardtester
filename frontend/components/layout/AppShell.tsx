...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Live product input drives the horizon — never hardcode Simulation End Days.
  const simDays = product?.simulation_end_days ?? market?.simulation_end_days ?? null;
  const simEnd =
    market?.simulation_end ??
    (asof && simDays != null ? addCalendarDaysIso(asof, Number(simDays)) : null);
  const horizonReady = isDeskHorizonMeta(market);
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-mesh">
                ...
                return (
                  ...
                  >
                    <span className="market-meta-card__label">
                      <Icon className="h-3.5 w-3.5 shrink-0 text-[var(--ar-gold-dark)]" />
                      {m.label}
                    </span>
                    <strong className="market-meta-card__value">{m.value}</strong>
                  </motion.div>
                );
              })}
            </div>
      ...
      <footer className="border-t border-[var(--ar-border)] py-3 text-center text-xs text-[var(--ar-subtle)]">
        Gift City AIF Forwardtester · Geometric Brownian Motion from today&apos;s Nifty close
      </footer>
    </div>
  );
}
